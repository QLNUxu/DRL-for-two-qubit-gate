...
class YY(Environment):
    ...
    def execute(self, actions):
        ...
        if 0<systime<=self.tf1:# U1 otime U2
            
            Rabi1 = actions[0]*self.strength*2
            Detuning1 = 2*(actions[1]-0.5)*self.strength*1.6
            ...
            Rabi2 = actions[2]*self.strength*2
            Detuning2 = 2*(actions[3]-0.5)*self.strength*1.6
        ...
        elif self.tf1<systime<=self.tf2:# eye otime U2
        ...
        elif self.tf2<systime<=self.tf2+self.tfd:# U
            ...
            Rabi2 = actions[2]*self.strength*2
            Detuning2 = 2*(actions[3]-0.5)*self.strength*2.6
            H2 = 0.5*( Rabi2*(1+self.lam_Omega)*np.cos(self.omega*systime)*sigmax() + Rabi2*(1+self.lam_Omega)*np.sin(self.omega*systime)*sigmay() + Detuning2*(1+self.lam_Delta)*sigmaz())
        
            J = 2*(actions[4]-0.5)*self.strength*1.7
            H3 = 0.5 * J*(1+self.lam_J) * tensor(sigmaz(),sigmaz())
        ...
        elif self.tf2+self.tfd<systime<=self.tf2+self.tfd+self.tf4:# U3 otimes U4
            
            Rabi1 = actions[0]*self.strength*2
            Detuning1 = 2*(actions[1]-0.5)*self.strength*1.9
            H1 = 0.5*( Rabi1*(1+self.lam_Omega)*np.cos(self.omega*systime)*sigmax() + Rabi1*(1+self.lam_Omega)*np.sin(self.omega*systime)*sigmay() + Detuning1*(1+self.lam_Delta)*sigmaz())      
